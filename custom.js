document.addEventListener("DOMContentLoaded", function () {
    ...
    function replacePlaceholders(ip) {
        document.querySelectorAll(`[data-placeholder="${placeholder}"]`).forEach(element => {
            element.innerHTML = ip || "Enter Pod IP";
        });

        document.querySelectorAll(`a[href*="${placeholder}"]`).forEach(link => {
            link.href = link.href.replace(placeholder, ip || "Enter Pod IP");
        });
    }
    ...
    // Target the form container and add the form if it exists
        ...
        formContainer.innerHTML = `
            <form id="set-pod-ip">
                ...
                <input style="border: 1px solid #ccc; outline: 1px solid #ccc; height: 1.5rem; padding:0px; padding-left:3px; padding-right:3px;" 
                       type="text" id="pod_ip" value="${storedIP || ""}">
